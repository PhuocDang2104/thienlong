...
export default function GuestsPage() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.18em] text-primary"><span className="h-0.5 w-7 bg-brand-red"/>Quản lý khách</p><h1 className="text-3xl font-bold tracking-[-.035em]">Khách mời</h1></div><div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={() => void resource.refresh()} disabled={resource.loading}><RefreshCw className={resource.loading ? "animate-spin" : ""}/>Làm mới</Button><Button variant="secondary" asChild><Link href="/admin/import"><FileUp/>Nhập file</Link></Button><Button onClick={() => { setSuccess(null); setCreating(true); }}><UserPlus/>Thêm khách</Button></div></div>
    {success && <Alert tone="success">{success}</Alert>}{resource.error && <Alert>{resource.error}</Alert>}
    <div className="card-shell overflow-hidden rounded-2xl">
      <div className="border-b border-border bg-white px-4 pt-4 sm:px-5">
        <div role="tablist" aria-label="Chế độ danh sách khách" className="inline-flex rounded-xl bg-blue-50/80 p-1 shadow-inner">
          <button role="tab" aria-selected={mode === "invitation"} onClick={() => selectMode("invitation")} className={`min-h-10 rounded-lg px-4 text-sm font-semibold transition ${mode === "invitation" ? "bg-white text-primary shadow-[0_4px_12px_rgba(36,74,165,.13)]" : "text-muted hover:text-primary"}`}>Thư mời</button>
          <button role="tab" aria-selected={mode === "checkin"} onClick={() => selectMode("checkin")} className={`min-h-10 rounded-lg px-4 text-sm font-semibold transition ${mode === "checkin" ? "bg-primary text-white shadow-[0_5px_14px_rgba(36,74,165,.22)]" : "text-muted hover:text-primary"}`}>Danh sách check-in</button>
        </div>
        <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-border/70 py-4"><div className="relative min-w-52 flex-1 md:max-w-sm"><label htmlFor="guest-search" className="sr-only">Tìm khách mời</label><Search className="absolute top-3 left-3 size-4 text-slate-400"/><Input id="guest-search" placeholder="Tên, công ty, email, điện thoại…" value={search} onChange={(event) => setSearch(event.target.value)} className="pl-9"/></div>
          {mode === "invitation" ? <Select aria-label="Lọc phản hồi" value={rsvp} onChange={(event) => { setRsvp(event.target.value); setPage(1); }}><option value="">Tất cả phản hồi</option><option value="accepted">Đã xác nhận</option><option value="pending">Chưa phản hồi</option><option value="declined">Từ chối</option></Select> : <><Select aria-label="Lọc check-in" value={checkin} onChange={(event) => { setCheckin(event.target.value); setPage(1); }}><option value="">Tất cả khách xác nhận</option><option value="not_checked_in">Chưa đến</option><option value="checked_in">Đã check-in</option></Select><span className="ml-auto hidden items-center gap-2 text-xs font-medium text-emerald-700 md:flex"><span className="live-dot size-1.5 rounded-full bg-emerald-500"/>Tự động cập nhật</span></>}
