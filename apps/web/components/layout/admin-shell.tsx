...
function Navigation({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <>
    <Link href="/" className="mb-10 block px-2 py-1"><BrandLogo priority className="w-full"/></Link>
    <p className="mb-3 px-3 text-[10px] font-bold uppercase tracking-[0.16em] text-[#486394]">Quản trị sự kiện</p>
    <nav aria-label="Điều hướng quản trị" className="space-y-1">{navigation.map(({ href, label, Icon }) => <Link key={href} href={href} onClick={onNavigate} aria-current={pathname === href ? "page" : undefined} className={cn("relative flex items-center gap-3 overflow-hidden rounded-lg px-3.5 py-2.5 text-sm font-semibold text-[#173a7a] transition-colors", pathname === href ? "bg-[#edf2fa] text-[#123b88] before:absolute before:inset-y-2 before:left-0 before:w-0.5 before:bg-brand-red" : "hover:bg-slate-50 hover:text-primary")}><Icon className="size-[18px]"/>{label}</Link>)}</nav>
...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return <AdminContext.Provider value={{ token: auth.session.access_token, event: auth.event, version, invalidate: () => setVersion((value) => value + 1), streamStatus: stream.status }}>
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-slate-200 bg-white px-5 py-7 lg:flex"><Navigation/></aside>
    <div className="brand-page min-h-dvh lg:pl-64">
      <header className="sticky top-0 z-20 flex min-h-16 flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white px-5 py-3 md:px-8">
        <div className="flex items-center gap-3"><Dialog.Root open={drawerOpen} onOpenChange={setDrawerOpen}><Dialog.Trigger asChild><Button variant="ghost" size="icon" className="lg:hidden" aria-label="Mở menu"><Menu/></Button></Dialog.Trigger><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-40 bg-slate-950/45"/><Dialog.Content className="fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-slate-200 bg-white px-5 py-7 shadow-[8px_0_24px_rgba(15,23,42,.12)]"><Dialog.Title className="sr-only">Điều hướng quản trị</Dialog.Title><Dialog.Description className="sr-only">Các khu vực vận hành sự kiện</Dialog.Description><Dialog.Close aria-label="Đóng menu" className="absolute top-3 right-3 rounded-md p-2 text-[#173a7a] hover:bg-slate-100 hover:text-primary"><X className="size-4"/></Dialog.Close><Navigation onNavigate={() => setDrawerOpen(false)}/></Dialog.Content></Dialog.Portal></Dialog.Root><span className="text-sm font-bold text-foreground">{activeTitle}</span><span className="hidden text-slate-300 md:inline">/</span><span className="hidden max-w-72 truncate text-xs text-muted md:inline">{auth.event.name}</span></div>
